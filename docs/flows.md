...
sequenceDiagram
    ...
    participant Popup as Sign-in window
    ...
    Person->>Vue: Login with OneID
    Vue->>Popup: window.open, in the same tick as the click
    Popup->>KC: GET /auth with PKCE challenge, state, nonce, kc_idp_hint=oneid
    Note over KC: kc_idp_hint skips Keycloak's own sign-in page
    KC-->>Popup: 303 to OneID with response_type=one_code and state
    Person->>OneID: signs in inside the window with e-signature, Mobile-ID or password
    OneID-->>Popup: 302 to Keycloak's broker endpoint with code and state
    Popup->>KC: GET /broker/oneid/endpoint with code and state
    ...
    KC-->>Popup: 302 to /auth-callback.html with a Keycloak code, state and iss
    Popup->>Vue: BroadcastChannel platform-login with code, state and iss
    Popup->>Popup: closes itself
    Note over Vue: accepts only its own state and the realm's iss
    Vue->>KC: POST /token with the code and the PKCE verifier
    KC-->>Vue: access token, refresh token, ID token
    Note over Vue: checks the ID token's nonce, then keycloak-js takes the tokens
...
The redirects inside the sign-in window, for `akarimov`:

```
303 http://localhost:8190/realms/platform/protocol/openid-connect/auth   with kc_idp_hint=oneid
303 http://localhost:8190/realms/platform/broker/oneid/login
200 http://localhost:8191/sso/oauth/Authorization.do                    the person signs in here
302 http://localhost:8191/sso/oauth/select
302 http://localhost:8190/realms/platform/broker/oneid/endpoint
->  http://localhost:5174/auth-callback.html?state=...&iss=...&code=...
```

Two short Keycloak steps, first-broker-login and after-first-broker-login, appear between the
endpoint and the callback page on a person's **first** login only: that is where Keycloak creates
the user. On every later login the provider updates them instead, recognising the person by their PIN.

The authorization request Keycloak sends the window on to:
...
The app never navigates away. `login()` in `frontend/src/keycloak.js`:

1. **Opens the window in the same tick as the click.** After any `await`, browsers treat
   `window.open` as unsolicited and block it.
2. **Sends it to Keycloak** with a PKCE challenge, a random `state` and `nonce`, and
   `kc_idp_hint=oneid`, which takes it past Keycloak's own sign-in page straight to OneID.
3. **Waits for `/auth-callback.html`.** That page hands the code, `state` and `iss` to the app over a
   `BroadcastChannel`, removes the code from its own history, and closes. A `BroadcastChannel`
   reaches the app even if a page on the way sends `Cross-Origin-Opener-Policy`, which would
   silently break `window.opener`.
4. **Checks, then redeems.** It ignores an answer carrying any other `state`, refuses one whose `iss`
   is not the realm, redeems the code with the PKCE verifier it kept in memory, and refuses an ID
   token whose `nonce` is not its own.
5. **Hands the tokens to a fresh keycloak-js instance**, which refreshes them and builds the logout
   URL. An instance can be initialised only once, so each sign-in gets a new one.

The OneID token never reaches the browser. The code the window carries is useless without the
verifier, which never leaves the app window's memory, and no token is ever written to storage.

**Developer accounts (password)** opens the same window without `kc_idp_hint`, so Keycloak's own
sign-in page appears for the development users. If the browser blocks the window, or the person
closes it before finishing, the app says so and stays signed out.
